...
int main() {
  ...
  if (epoll_fd == -1) {
    logger(LOG_ERROR, "main()", "xps_loop_create() failed");
    return -1;
  }
  // printf("[INFO] Event loop created with epoll FD: %d\n", epoll_fd);
  // Init lists
  vec_init(&listeners);
  ...
  for (int port = 8001; port <= 8003; port++) {
    ...
    if (listener == NULL) {
      logger(LOG_ERROR, "main()", "xps_listener_create() failed for port %u", port);
      return -1;
    }
